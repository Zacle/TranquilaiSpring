...
class JournalPromptController(private val favoriteService: FavoriteService) {

    @GetMapping("/favorites")
    fun favorites(
        @AuthenticationPrincipal user: GatewayUser,
    ): ResponseEntity<List<String>> =
        ResponseEntity.ok(favoriteService.getFavorites(user.id, ContentType.JOURNAL_PROMPT))
    ...
    fun toggleFavorite(
        ...
        ResponseEntity.ok(favoriteService.toggle(user.id, ContentType.JOURNAL_PROMPT, id))
}
