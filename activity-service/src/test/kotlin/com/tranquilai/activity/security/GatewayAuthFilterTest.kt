package com.tranquilai.activity.security
...
class GatewayAuthFilterTest {
    ...
    @Test
    fun `uses USER role when roles header is missing`() {
        val userId = UUID.randomUUID()
        val request = MockHttpServletRequest().apply {
            addHeader("X-User-Id", userId.toString())
            addHeader("X-User-Email", "user@example.com")
        ...
        assertEquals(listOf("ROLE_USER"), SecurityContextHolder.getContext().authentication.authorities.map { it.authority })
        verify(chain).doFilter(request, response)
    }
